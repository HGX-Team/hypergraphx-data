#!/usr/bin/env python3
"""Generate movies-cast-genre-multiplex from The Movies Dataset CSV files."""
...
DATASET_NAME = "movies-cast-genre-multiplex"
...
def build_hypergraph(raw_dir: Path) -> MultiplexHypergraph:
    ...
    hypergraph = MultiplexHypergraph(weighted=True)
    active_nodes = set()
    edge_records = []
    ...
    with (raw_dir / MOVIES_FILE).open(newline="", encoding="utf-8") as file_obj:
        for row in csv.DictReader(file_obj):
            ...
            for genre in genres:
                edge_metadata = {"layer": genre, **metadata}
                edge_records.append((cast, genre, edge_metadata))

    for node in sorted(active_nodes):
        hypergraph.add_node(node, metadata=actor_metadata.get(node, {"tmdb_person_id": node}))

    for cast, genre, edge_metadata in edge_records:
        hypergraph.add_edge(cast, layer=genre, metadata=edge_metadata)
    ...
    hypergraph.set_attr_to_hypergraph_metadata("layer_type", "genre")
    return hypergraph
