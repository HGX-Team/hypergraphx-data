#!/usr/bin/env python3
"""Generate contacts-hospital from local Sociopatterns raw contact data."""
...
DATASET_NAME = "contacts-hospital"
...
def build_hypergraph(raw_dir: Path) -> TemporalHypergraph:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line in handle:
            if not line.strip():
                continue
            timestamp_raw, source_raw, target_raw, source_status, target_status = line.split()
            timestamp = int(timestamp_raw)
            source = int(source_raw)
            target = int(target_raw)
            contacts_by_time.setdefault(timestamp, []).append((source, target))
            node_order.extend((source, target))
            node_metadata.setdefault(source, {"id": source, "status": source_status})
            node_metadata.setdefault(target, {"id": target, "status": target_status})
    hypergraph = TemporalHypergraph(weighted=False)
    for node in ordered_unique(node_order):
        hypergraph.add_node(node, metadata=node_metadata[node])
