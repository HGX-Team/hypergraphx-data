...
if __name__ == "__main__":
    dataset_path = "NDC-substances-full"
    hypergraph = load_timestamped_simplex_with_metadata(dataset_path)
    hypergraph.set_attr_to_hypergraph_metadata("name", "NDC-substances")
    hypergraph.set_attr_to_hypergraph_metadata("version", "1.0.0")
    save_hypergraph(hypergraph, "NDC-substances.json")
    save_hypergraph(hypergraph, "NDC-substances.hgx", binary=True)

    from hypergraphx.readwrite import load_hypergraph
    a = load_hypergraph("NDC-substances.json")
    print(a.get_hypergraph_metadata())
    print(len(a.get_nodes()))
    print(len(a.get_edges()))
    b = load_hypergraph("NDC-substances.hgx")
