#!/usr/bin/env python3
"""Generate contacts-primary-school from local Sociopatterns raw contact data."""
...
DATASET_NAME = "contacts-primary-school"
...
def load_node_metadata(path: Path) -> dict[int, dict]:
    metadata = {}
    with path.open("r", encoding="utf-8") as handle:
        for line in handle:
            if not line.strip():
                continue
            node_id, class_id, sex = line.split()
            metadata[int(node_id)] = {"id": int(node_id), "class": class_id, "sex": sex}
    return metadata


def build_hypergraph(raw_dir: Path) -> TemporalHypergraph:
    contacts_by_time, _ = load_pair_contacts(raw_dir / "primaryschool.csv")
    node_metadata = load_node_metadata(raw_dir / "metadata_ps.txt")
